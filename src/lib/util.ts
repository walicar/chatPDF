...
export async function createEmbeddings(texts: string[], indexName: string) {
...
export async function queryDoc(query:string, vectorStore: any) {
  try {
...
export async function getStore(indexName: string) {
  const fields = { openAIApiKey: process.env.OPENAI_API_KEY };
  const embeddings = new OpenAIEmbeddings(fields);
  const pineconeClient = new PineconeClient();
  try {
...
export function makeMessage(color:string, name:string, content:string) {
  const message = {
    color: color,
    name: name,
    content: content,
